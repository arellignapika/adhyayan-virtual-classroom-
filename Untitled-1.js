...
// const fileUpload = require('./routes/forpdf'); // Import the fileUpload module
...
app.post('/joinclass', encoder, (req, res) => {
  ...
  db.query("select * from classopt_pr where classcode  = ? ", [classcode], function (error, results, fields) {
    if (results.length > 0) {
      ...
      req.session.class_code=req.body.classcode ;
...
// teacher todo page
app.get("/submit_form", encoder , function (req, res) {
  console.log(req.session.user_idstu);
